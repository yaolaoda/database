...
public class MovieDAOImpl extends DAOBase implements MovieDAO {
	...
	public void selectMovie(int movieId) {
		...
		try{
			...
			sql="select ActingStaff.chineseName,MovieStaff.status,ActingStaff.staffid "
					+ "from Movie,MovieStaff,ActingStaff "
					+ "where Movie.movieid=MovieStaff.movieid and ActingStaff.staffid=MovieStaff.staffid and Movie.movieid=?";			
			ps=conn.prepareStatement(sql);		
			ps.setLong(1, movieId);
			rs=ps.executeQuery();
			int i=5;
